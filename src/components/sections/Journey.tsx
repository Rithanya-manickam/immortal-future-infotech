  ...
  return (
    <section id="journey" ref={wrap} className="relative h-[100svh] w-full overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 top-16 z-10 mx-auto max-w-[1400px] px-6">
        <div className="text-[10px] uppercase tracking-[0.4em] text-white/50">Section 02 — The Journey</div>
          ...
          {MILESTONES.map((m, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="font-mono text-xs uppercase tracking-[0.3em] text-white/50">{m.year}</span>
                <span className="h-2 w-2 rounded-full bg-[var(--brand-glow)] shadow-[0_0_16px_var(--brand-glow)]" />
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-[0.35em] text-white/40">Milestone {String(i + 1).padStart(2, "0")}</div>
                <h3 className="mt-2 text-2xl font-semibold text-white md:text-3xl">{m.title}</h3>
                <p className="mt-3 max-w-md text-sm leading-relaxed text-white/60">{m.body}</p>
              </div>
